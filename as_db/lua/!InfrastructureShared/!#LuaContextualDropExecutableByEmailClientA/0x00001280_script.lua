...
-- Command line: db 092023\as_db\lua\!InfrastructureShared\!#LuaContextualDropExecutableByEmailClientA\0x00001280_luac 
...
if (mp.IsPathExcludedForHipsRule)(l_0_2, "be9ba2d9-53ea-4cdc-84e5-9b1eeee46550") then
  return mp.CLEAN
end
                  ...
                  if IsSuspiciousFileExt(l_0_8) then
                    do
                      if l_0_8 == "dll" or l_0_8 == "js" or l_0_8 == "exe" then
                        ...
                        if (string.find)(l_0_2, "appdata\\local\\microsoft\\windows\\appcache\\%w+\\%d+\\.+%.js") ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, "appdata\\local\\packages\\microsoft%.outlookforwindows_[^\\]+\\ac\\inetcache\\[^\\]+\\.+%.js") ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, ".:\\windows\\microsoft%.net\\assembly\\gac_msil\\[^\\]+\\[^\\]+\\.+%.dll") ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, ":\\program files (x86)\\zoom\\zoom outlook plugin\\x64\\zoomoutlookplugin64.dll", 1, true) ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, l_0_11 .. "\\appdata\\local\\temp\\%d+\\intresource64%.dll") ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, l_0_11 .. "\\appdata\\local\\temp\\deployment\\[^\\]+\\[^\\]+\\caseninja%.dll") ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, l_0_11 .. "\\appdata\\local\\temp\\deployment\\[^\\]+\\[^\\]+\\federatedauthentication%.dll") ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, l_0_11 .. "\\appdata\\local\\temp\\deployment\\[^\\]+\\[^\\]+\\sharedresources%.dll") ~= nil then
                          return mp.CLEAN
                        end
                        if (string.find)(l_0_2, l_0_11 .. "\\appdata\\local\\temp\\deployment\\[^\\]+\\[^\\]+\\mssolvehelper%-databroker%.exe") ~= nil then
                          return mp.CLEAN
                        end
                      end
                      ;
